...
async def test_migrations_create_new_database_to_head(tmp_path: Path) -> None:
    ...
    assert version == "f466616b2c85"
    assert {"nameEn", "descriptionEn", "addressEn", "eventPlaceEn"} <= board_columns
...
async def test_migrations_stamp_legacy_database_without_deleting_rows(tmp_path: Path) -> None:
    ...
    async with target_engine.begin() as connection:
        ...
        await connection.execute(text('INSERT INTO "Board" ("boardId", name, category) VALUES (7, \'보존할 데이터\', \'관광지\')'))
    ...
    async with target_engine.connect() as connection:
        version = await connection.scalar(text("SELECT version_num FROM alembic_version"))
        row = (await connection.execute(text('SELECT "boardId", name FROM "Board"'))).one()
    assert version == "f466616b2c85"
    assert tuple(row) == (7, "보존할 데이터")
    await target_engine.dispose()
