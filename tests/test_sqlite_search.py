...
async def test_sqlite_search_uses_real_board_and_post_table_names(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async with engine.begin() as connection:
        ...
        await connection.execute(text('INSERT INTO "Board" ("boardId", name, category, description, image) VALUES (1, \'해운대 해수욕장\', \'관광지\', \'부산 해운대구 대표 해변\', \'https://example.com/haeundae.jpg\')'))
    ...
    assert boards["items"] == [{"sourceType": "Board", "sourceId": "1", "title": "해운대 해수욕장", "content": "부산 해운대구 대표 해변", "address": None, "imageUrl": "https://example.com/haeundae.jpg", "category": "관광지"}]
