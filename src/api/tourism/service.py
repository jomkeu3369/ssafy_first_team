...
from src.api.tourism.schema import AttractionCategory, AttractionResponse, FestivalResponse, FestivalStatus
...
def _to_attraction(item: dict[str, Any]) -> AttractionResponse:
    ...
    return AttractionResponse(content_id=_text(item.get("contentid")), name=name, category=_attraction_category(item), summary=summary, description="\n".join(value for value in details if value) or summary, image=_text(item.get("firstimage")), address=address)
...
def _to_festival(item: dict[str, Any], today: date) -> FestivalResponse:
    ...
    return FestivalResponse(content_id=_text(item.get("contentid")), name=name, status=_festival_status(start_date, end_date, today), place=place, period=period, start_date=start_date, end_date=end_date, image=_text(item.get("firstimage")), summary=summary)
