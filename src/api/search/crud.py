...
async def search_all(db: AsyncSession, query: str, page: int, size: int) -> SearchResponse:
    ...
    board_search = select(literal("BOARD").label("result_type"), Board.board_id.label("result_id"), Board.board_id.label("board_id"), Board.name.label("title"), Board.description.label("description"), Board.image.label("image"), Board.category.label("category")).where(or_(Board.name.ilike(pattern), Board.name_kr.ilike(pattern), Board.name_en.ilike(pattern), Board.description.ilike(pattern), Board.description_kr.ilike(pattern), Board.description_en.ilike(pattern), Board.address.ilike(pattern), Board.address_en.ilike(pattern), Board.event_place.ilike(pattern), Board.event_place_en.ilike(pattern), Board.category.ilike(pattern), Board.category_kr.ilike(pattern), Board.category_en.ilike(pattern)))
