...
부산 관광 정보와 커뮤니티 데이터를 활용해 질문에 답하는 AI API입니다. FastAPI는 애플리케이션 진입점과 HTTP 라우팅만 담당하고, 검색·판단·답변 생성 흐름은 `agent/` 패키지의 LangGraph 기반 RAG 파이프라인으로 분리할 예정입니다.
...
│  ├─ models.py            # ChatOpenAI 및 임베딩 모델 생성
│  ├─ config.py            # AI·LangSmith 환경 설정
│  ├─ state.py             # LangGraph 상태 정의
│  ├─ graph.py             # 노드와 엣지 조립 및 그래프 컴파일
│  ├─ nodes.py             # 분류·검색·검증·생성 노드
│  ├─ retrieval.py         # SQL/키워드/벡터 검색 연결
│  └─ prompts.py           # 근거 기반 답변 프롬프트
├─ tests/
├─ .env.example
├─ pyproject.toml
├─ uv.lock
...
`models.py`는 LLM 및 임베딩 모델 설정에 사용하고, API 데이터 모델은 `schemas.py`에 둡니다. SQLAlchemy 데이터 모델은 다른 백엔드 영역에 배치합니다.
...
- 공통 미들웨어와 예외 처리기 등록
- 헬스 체크 등록 또는 연결
- `agent.router`를 `include_router()`로 연결
...
데이터베이스 세션이나 ORM 모델을 `agent/` 전역 상태로 직접 보관하지 않습니다. FastAPI 의존성 주입 또는 별도 검색 서비스 인터페이스를 통해 요청 단위로 전달합니다.
