...
로컬 `data/localhub.db`를 직접 번역하려면 다음 명령을 사용합니다. 실행 전에 같은 폴더에 타임스탬프가 포함된 SQLite 백업 파일을 자동 생성합니다.

```powershell
uv run python scripts/translate_local_boards.py
```

다른 SQLite 파일을 지정하거나 일부만 시험하려면 다음 옵션을 사용합니다.

```powershell
uv run python scripts/translate_local_boards.py --database-path "C:/path/localhub.db" --max-records 100
```
